...
function buildUserProfilePayloadFromAuthRecord(user) {
  ...
  return {
    ...
    name,
    role: "user",
    status: { isActive: true, isBlocked: false, isTrial: false },
    limits: { storageMB: 50, maxFiles: 100, maxCourses: 20, maxNotes: 200 },
    usage: {
      storageUsedMB: 0,
      fileCount: 0,
      courseCount: 0,
    ...
    features: { advancedAnalytics: true, aiNotes: true, exportPDF: true },
    createdAt: now,
    lastLogin: now,
  };
}
...
exports.aggregateUserUsage = onDocumentWritten("users/{userId}/data/{moduleName}", async (event) => {
  const { userId, moduleName } = event.params;

  // Only trigger aggregation for asset and storage-relevant module changes
  ...
  if (!relevantModules.has(moduleName)) {
    return;
  }
