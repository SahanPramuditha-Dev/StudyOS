...
const ResourceItem = ({
  ...
  onToggleSelect,
  onDragStart,
  onOpen // new prop for analytics
}) => {
  const [previewOpen, setPreviewOpen] = useState(false);
  const url = res.url || res.isLocal ? res.url : '#';
      ...
      >
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-2">
            ...
            <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-primary-500 group-hover:bg-primary-500 group-hover:text-white transition-all shadow-sm relative overflow-hidden">
              {getIcon()}
              <button
                onClick={() => setPreviewOpen(true)}
                className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center bg-black/20 hover:bg-black/30"
                aria-label={`Preview ${res.name}`}
                title="Preview"
              >
